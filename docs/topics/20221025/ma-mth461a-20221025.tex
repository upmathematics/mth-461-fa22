...
\hypersetup{
  pdftitle={Continuous Random Variables (CRVs)},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Continuous Random Variables (CRVs)}}
\usepackage{etoolbox}
\makeatletter
...
\item
  Use the definitions of probability to verify the following probability density functions.

  \begin{enumerate}
  \def\labelenumii{\alph{enumii}.}
  \item
    Uniform Distribution \(\displaystyle f(x;b,a) = \frac{1}{b-a}\)
  \item
    Exponential Distribution \(\displaystyle f(x;\lambda) = \lambda e^{-\lambda x}\)
  \end{enumerate}
\item
  Let \(X \sim Exp(p)\) be a continuous exponential random variable with parameter \(\lambda\). Show that the mean of \(X\) is \(\displaystyle \frac{1}{\lambda}\), and the variance of \(X\) is \(\displaystyle \frac{1}{\lambda^2}\).
