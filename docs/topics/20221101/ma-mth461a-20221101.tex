...
\hypersetup{
  pdftitle={Covariance \& Correlation for CRVs},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Covariance \& Correlation for CRVs}}
\usepackage{etoolbox}
\makeatletter
...
\item
  Draw the integration region \(R_{XY}\) and find the constant \(c\).
\item
  Find the marginal distributions \(f_X(x)\) and \(f_Y(y)\), and determine if \(X\) and \(Y\) are independent.
\item
  Find the \(Cov(X,Y)\), and determine if \(X\) and \(Y\) are independent.
\end{enumerate}
