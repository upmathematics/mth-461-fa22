...
\hypersetup{
  pdftitle={Joint \& Marginal Distributions for CRVs},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Joint \& Marginal Distributions for CRVs}}
\usepackage{etoolbox}
\makeatletter
...
\item
  Let \(X \sim Bernoulli(p)\) and \(Y \sim Bernoulli(q)\) be independent, where \(0 < p\),\(q < 1\). The pmf for a Bernoulli distribution is given below.
  ...
  \item
    Find the expected value and variance of the jointly discrete random variable using the joint PMF.
  \end{enumerate}
\item
  ...
  \tightlist
  \item
    Find \(R_{XY}\) and show it in the \(x-y\) plane. This means the integration region.
  \item
    Find the constant \(c\).
  \item
    Find marginal PDFs, \(f_{X}(x)\) and \(f_{Y}(y)\).
  \item
    Find \(P\left(Y \le \frac{X}{2}\right)\).
  \item
    Find \(P\left(Y \le \frac{X}{4} | Y \le \frac{X}{2}\right)\).
  \end{enumerate}
\end{enumerate}
