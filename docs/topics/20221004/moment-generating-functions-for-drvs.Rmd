...
This is what is known as the **2nd moment** or the **variance**.
...
If $Y$ is a $Uniform(0,1)$ random variable. 

a. Find the MGF.

$$M_Y(s) = E\left[e^{sY}\right]= \int_0^1 e^{sy}\,dy = \frac{e^s − 1}{s}.$$
 
Note that we always have $M_Y(0)=E[e^{0Y}]=1$, thus $M_Y(s)$ is also well-defined for all $s \in \mathbb{R}$.

b. Find $E[Y^k]$ using $M_Y(s)$.

$$M_Y(s) = \frac{e^s−1}{s} = \frac{1}{s} \left( \sum_{k=0}^{\infty} \frac{s^k}{k!} − 1 \right) = \frac{1}{s} \sum_{k=0}^{\infty} \frac{s^k}{k!} = \sum_{k=0}^{\infty} \frac{s^{k−1}}{k!} = \sum_{k=0}^{\infty} \frac{1}{k+1} \frac{s^k}{k!}.$$

Thus, the coefficient of $\frac{s^k}{k!}$ in the Taylor series for $M_Y(s)$ is $\frac{1}{k+1}$, so

$$E\left[X^k\right] = \frac{1}{k+1}.$$
