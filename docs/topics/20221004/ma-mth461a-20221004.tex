...
\hypersetup{
  pdftitle={Moment generating Functions for DRVs},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Moment generating Functions for DRVs}}
\usepackage{etoolbox}
\makeatletter
...
\item
  Let \(X \sim Bernoulli(p)\) with parameter \(p\) for some \(0 \le p \le 1\). Find the moment generating function \(M_X(s)\) and the nth moment \(E\left[ X^n \right]\).

  The pmf for a Bernoulli distribution is given by
  \[P(X = x; p) = p^x (1-p)^{1-x} \text{ for } x \in \{0,1\}.\]
\item
  Let \(X \sim Poisson(\lambda)\) with parameter \(\lambda\). Find the moment generating function of \(X\), \(M_X(s)\), and the nth moment, \(E\left[ X^n \right]\).
