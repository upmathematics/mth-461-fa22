...
\hypersetup{
  pdftitle={Covariance and Correlation for DRVs},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Covariance and Correlation for DRVs}}
\usepackage{etoolbox}
\makeatletter
...
\item
  Suppose that \(X\) and \(Y\) are random variables, and \(Z = aX + bY\) for some scalar constants \(a\) and \(b\). Use the definition and properties of the covariance to compute the formulas of the following expressions.
  ...
  \end{enumerate}
\item
  Let \(X\) and \(Y\) be two random variables associated with the throws of two ordinary dice. In both cases the mean is \(E[X] = E[Y] = \frac{7}{2}\) and the variance is \(Var(X) = Var(Y) = \frac{35}{12}\). Suppose the two dice behave so as to give only the (equiprobable) pairs (1,6), (2,5), (3,4), (4,3), (5,2) and (6,1).
