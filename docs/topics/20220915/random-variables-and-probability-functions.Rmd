...
## Tossing a Fair Coin $\mathbf{n = 20}$ Times {.bigger}
...
library(ggplot2)
p<-ggplot(df, aes(x=x, y=probability)) +
  geom_point(size=4) + ggtitle("PMF of the Binomial Distribution") +
...
The probability of observing **exacty** `r x_7` heads is
$$P(X = `r x_7`) = `r p_7`$$
</div>
...
The probability of observing **at least** `r x_7[1]` heads is
$$P(X \ge `r x_7[1]`) = \sum_{x=7}^{20} P(X = x) = `r p_al7_sum`$$
</div>

## Discrete R.V. versus Continuous Random Variables
...
[Mini-Assignment: Bayes Theorem](ma-mth461a-20220915.pdf){target="_blank"}
