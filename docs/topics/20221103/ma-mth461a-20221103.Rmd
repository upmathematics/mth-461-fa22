---
title: '**Conditioning & Independence for CRVs**'
...
1. Suppose $X$ is a continuous random variable with pdf shown below.
$$
f_X(x) = 
\begin{cases}
cx^2, & 0 \le x \le 3 \\
...
$$
    a. Find c.
    b. Find the conditional pdf and cdf of $X$ given $2 \le X < 3$.

2. Suppose $X$ and $Y$ are jointly continuous random variables with joint pdf shown below.
$$
f_{XY}(x,y) = 
\begin{cases}
...
\end{cases}
$$
    a. Find the conditional pdf and cdf of $X$ given $Y = y$, where $-1 \le y \le 1$.
