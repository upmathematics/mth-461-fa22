...
\hypersetup{
  pdftitle={Conditioning \& Independence for CRVs},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{\textbf{Conditioning \& Independence for CRVs}}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Mini-Assignment 2022-11-03 - MTH 461 - Fall 2022}
\author{}
\date{\vspace{-2.5em}}
...
\tightlist
\item
  Suppose \(X\) is a continuous random variable with pdf shown below.
  \[
  f_X(x) = 
  \begin{cases}
  cx^2, & 0 \le x \le 3 \\
  ...
  \item
    Find c.
  \item
    Find the conditional pdf and cdf of \(X\) given \(2 \le X < 3\).
  \end{enumerate}
\item
  Suppose \(X\) and \(Y\) are jointly continuous random variables with joint pdf shown below.
  \[
  f_{XY}(x,y) = 
  \begin{cases}
   \frac{1}{\pi} & (x,y) \in x^2 + y^2 \le 1\\
   0 & \text{otherwise} \\
  \end{cases}
  \]
  ...
  \tightlist
  \item
    Find the conditional pdf and cdf of \(X\) given \(Y = y\), where \(-1 \le y \le 1\).
  \item
    Find \(E[X|Y=y]\) and \(Var(X|Y=y)\).
  \end{enumerate}
\end{enumerate}
